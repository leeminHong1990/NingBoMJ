...
var GameRoomUI = UIBase.extend({
	...
	init_curplayer_panel:function(){
		...
		if (player.curGameRoom.player_num == 3) {
            var direction_list = ["east", "south", "west"];
			this.directionList = [];
			this.cur_player_panel.getChildByName("direct_img2").setVisible(false)
            for(var i = 0; i < 3; i++) {
                this.directionList[(player.curGameRoom.dealerIdx + i + 3) % 3] = direction_list[i];
            }
            cc.log("directionList:",this.directionList);
			for(var i = 0; i < 3; i++){
				var index = i < 2 ? i : 3;
			...
			var direction_list = ["east", "south", "west", "north"];
            this.directionList = [];
            for(var i = 0; i < 4; i++) {
                this.directionList[(player.curGameRoom.dealerIdx + i + 4) % 4] = direction_list[i];
            }
			cc.log("directionList:",this.directionList);
			for(var i = 0; i < 4; i++){
  ...
  //   	var curPlayNum = 0
		...
		if(idx == 0){
			...
			for(var i = 0; i < 45; i++){
				...
				if(player.curGameRoom.discardTilesList[serverSitNum][i] > 0){
					...
					tile_img.setVisible(true);
					if(player.curGameRoom.kingTiles.indexOf(player.curGameRoom.discardTilesList[serverSitNum][i]) >= 0){
						var kingtilemark_img = ccui.ImageView.create("res/ui/GameRoomUI/kingtilemark.png");
	...
	init_give_up_panel:function(){
		this.give_up_panel = this.rootUINode.getChildByName("give_up_panel");
		var player = h1global.entityManager.player();
		var self = this;
		this.give_up_panel.getChildByName("continue_btn").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					player.confirmGiveUp(const_val.NOT_GIVE_UP)
					self.hide_give_up_panel()
				}
			});

		this.give_up_panel.getChildByName("give_up_btn").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					player.confirmGiveUp(const_val.GIVE_UP)
					self.hide_give_up_panel()
				}
			});
		if (player.curGameRoom.giveUpState == const_val.WAIT_GIVE_UP) {
			this.show_give_up_panel()
		}
	},

	show_give_up_panel:function(){
		this.give_up_panel.setVisible(true)
	},
	...
	hide_give_up_panel:function(){
		if(!this.is_show) {return;}
		this.give_up_panel.setVisible(false);
	},
