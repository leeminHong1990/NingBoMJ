...
cutil.getHandTileQuantityWindDragons = function (handTilesButKing, kingTilesNum, p_wind, prevailing_wind) {
    ...
    for(var i = 0; i < temp_list.length; i++){
        if(handTilesButKing.indexOf(temp_list[i]) >= 0){
            ...
            if(tileDict[temp_list[i]] == 1 && kingTilesNum >= 2){
                handTilesButKing.push(temp_list[i]);
                handTilesButKing.push(temp_list[i]);
                kingTilesNum -= 2;
            }else if(tileDict[temp_list[i]] == 2 && kingTilesNum >= 1){
                handTilesButKing.push(temp_list[i]);
                kingTilesNum -= 1;
            }
        }
    ...
    if (cutil.meld_with_pair_need_num(handTilesButKing, {}) <= kingTilesNum) {
        ...
        cc.log("handTilesButKing:", handTilesButKing);
        quantity += cutil.getHandTileQuantity(handTilesButKing, p_wind, prevailing_wind);
    }
