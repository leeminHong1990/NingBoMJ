	...
	for t in tile2NumDict:
		if tile2NumDict[t] >= 3:
			...
			if t == p_wind:				#位风对应的 杠/刻
				quantity += 1
			if t in const.DRAGONS:	#中发白对应的 杠/刻
				quantity += 1
	return quantity

#手牌(财神优先配给圈风位风中发白后) 台数
def getHandTileQuantityWindDragons(handTilesButKing, kingTilesNum, p_wind, prevailing_wind):
	temp_list = [prevailing_wind, p_wind, const.DRAGON_RED, const.DRAGON_GREEN, const.DRAGON_WHITE]	#圈风位风中发白
	temp_handTilesButKing = copy.deepcopy(handTilesButKing)
	temp_kingNum = kingTilesNum
	is_run_temp_list = False	#是否执行了utility.getHandTileQuantity,防止重复计算台数
	quantity = 0
	for i in temp_list:
		if i in handTilesButKing:	#手牌中是否含有圈风位风中发白
			tileDict = getTile2NumDict(handTilesButKing)
			if tileDict[i] == 1 and kingTilesNum >= 2:
				handTilesButKing.append(i)
				handTilesButKing.append(i)
				kingTilesNum -= 2
			elif tileDict[i] == 2 and kingTilesNum >= 1:
				handTilesButKing.append(i)
				kingTilesNum -= 1
	if meld_with_pair_need_num(handTilesButKing, {}) <= kingTilesNum:	#财神转换之后的手牌是否可以胡
		is_run_temp_list = True
		DEBUG_MSG("handTilesButKing:{}".format(handTilesButKing))
		quantity += getHandTileQuantity(handTilesButKing, p_wind, prevailing_wind)
	handTilesButKing = temp_handTilesButKing
	kingTilesNum = temp_kingNum
	if not is_run_temp_list:
		quantity += getHandTileQuantity(handTilesButKing, p_wind, prevailing_wind)
	return quantity
...
def getWreathQuantity(wreaths, p_wind):
	...
	for t in wreaths:
		if t in const.SEASON:
			...
			flowerList[index] = 1
	quantity = 0
	if sum([i for i in seasonList]) + sum([i for i in flowerList]) == 8: #春夏秋冬 梅兰竹菊 8张齐
		quantity += 8
	else:
