...
class iRoomOperation(object):
	...
	def createRoom(self, player_num, pay_mode, win_quantity, game_round, king_num, is_agent):
		DEBUG_MSG("create room, {0}, {1}, {2}, {3}, {4}, {5}".format(player_num, pay_mode, win_quantity, game_round, king_num, is_agent))
		if game_round <= 0:
			return
	...
	def notifyPlayerGiveUp(self):
		DEBUG_MSG('notifyPlayerGiveUp')
		if getattr(self, 'client', None):
			self.client.notifyPlayerGiveUp()

	def confirmGiveUp(self, isGiveUp):
		if self.room:
			self.room.confirmGiveUp(self, isGiveUp)

	def unlockPongPlayerOperation(self):
		if getattr(self, 'client', None):
			self.client.unlockPongPlayerOperation()
